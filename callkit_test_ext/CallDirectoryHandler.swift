...
////  callkit_test_ext
////
////  Created by Krrish Sehgal on 22/11/24.
////
//
import Foundation
import CallKit
...
class CallDirectoryHandler: CXCallDirectoryProvider {
    override func beginRequest(with context: CXCallDirectoryExtensionContext) {
        print("CallDirectoryHandler: beginRequest called")
        
        // Add dummy blocked numbers
        let blockedNumbers: [Int64] = [1234567890]
        for number in blockedNumbers {
            context.addBlockingEntry(withNextSequentialPhoneNumber: number)
        }

        // Add dummy identified numbers
        let identifiedNumbers: [(Int64, String)] = [(9876543210, "Spam Caller")]
        for (number, label) in identifiedNumbers {
            context.addIdentificationEntry(withNextSequentialPhoneNumber: number, label: label)
        }

        context.completeRequest()
        print("CallDirectoryHandler: Request completed")
    }
